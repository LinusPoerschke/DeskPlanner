...
<head>
  ...
  <style>
    ...
    .date {
      font-size: 16px;
      color: #666; /* Optional, um das Datum etwas abzud�mpfen */
    }
    ...
    .logout-button {
      padding: 10px 20px;
      font-size: 16px;
      background-color: #ff4d4d; /* Auff�llige Farbe */
      color: white;
      border: none;
      border-radius: 5px;
      cursor: pointer;
    ...
    .status-left {
      flex: 1; /* Links nimmt den verf�gbaren Platz ein */
    }

    .status-right {
      flex: 1; /* Rechts nimmt den verf�gbaren Platz ein */
    }
    ...
    /* Container f�r den Timer und die Stoppuhr */
    .timercontainer {
      padding: 20px 40px;
      display: flex;
  ...
  <div class="page-content">
    ...
    function getLED() {
      ...
      if (file_exists($filePath)) {
          ...
          // �berpr�fen, ob der Wert in der Datei '1' ist
          if ($ledStatus === '1') {
    ...
    function getSocket() {
      ...
      if (file_exists($filePath)) {
          ...
          // �berpr�fen des Wertes in der Datei
          if ($socketStatus === '1') {
  ...
  ?>
   <div class="status-container">
    ...
    function getMondayOfCurrentWeek() {
        ...
        $currentDate->modify('-' . ($dayOfWeek - 1) . ' days'); // Gehe zur�ck auf den Montag
        return $currentDate;
    }
    ...
    // Funktion zum Pr�fen, ob das Datum in der aktuellen Woche liegt
    ...
    // Erstelle eine Liste der Wochentage von Montag bis Sonntag f�r die aktuelle Woche
    $monday = getMondayOfCurrentWeek();
    $weekDays = [];
    
    for ($i = 0; $i < 7; $i++) {
        // Jeden Tag einzeln erstellen, um das $monday-Objekt nicht zu ver�ndern
        $day = clone $monday;
        $day->modify('+' . $i . ' days');
        $weekDays[] = $day;
    }
    ...
    foreach ($exercises as $exercise) {
        ...
        // Pr�fen, ob die Aufgabe in der aktuellen Woche liegt
    ...
    // Formular zum Hinzuf�gen einer neuen Aufgabe
